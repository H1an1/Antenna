...
- [x] **24h 过期字段** — schema 里有 `expires_at`（pg_cron 自动清理见未完成）
...
- [ ] **agent 匹配逻辑升级** — Edge Function 关键词匹配 → agent 直接判断（砍掉中间层）
- [ ] **RLS 策略** — Supabase Row Level Security，防止读别人的 match
- [ ] **真机 GPS 测试** — 扫码跑一遍完整流程
